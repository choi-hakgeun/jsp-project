	...
	<div class="container" style="background: rgba(255,255,255,0.9); height: 700px; border-radius: 10px 10px 10px 10px;" >
		...
		<div class="row" id="bdCenter">
			<c:forEach var="vo" items="${BDlist }">
				<div class="item" onclick="viewE('${vo.n_serial}')">
					<div class="col-xs-2">
	...
	<div id="paging">
		<div align="center">
			<c:if test="${p.nowPage > p.blockSize }">
				<input type="button" value="처음" onclick="goPageE(1)">
			</c:if>
			<c:forEach var="i" begin="${p.startPage }" end="${p.endPage }">
				<input type="button" value="${i }" ${(i==p.nowPage)? "class='here'" : "" } onclick="goPageE(${i})">
			</c:forEach>
			<c:if test="${p.nowPage < p.totPage }">
				<input type="button" value="다음" onclick="goPageE(${p.nowPage+1})">
			</c:if>
		</div>
	</div>
	</div>
